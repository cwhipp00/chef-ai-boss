-- Create organization for existing user manually
DO $$
DECLARE
  org_id uuid;
  org_name text;
  user_id uuid := 'd9c01862-16fe-4009-a615-9c00c127c399';
BEGIN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM organization_members om WHERE om.user_id = user_id
  ) THEN
    -- Create organization
    ...
    RAISE NOTICE 'Created organization % for user %', org_name, user_id;
  END IF;
END $$;